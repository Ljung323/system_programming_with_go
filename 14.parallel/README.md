...
基本的にgoではチャネルとselectがあれば並列処理の実装には事足りるが、他言語からの書き換えなどを考えたとき、go独自の仕組みで実装するのは大変
→他言語での仕組みと似たようなsyncパッケージが提供されている

select: 複数チャネルの実行時に、受け取ったチャネルによって処理を変え、goroutineの実行をブロックする機能

ビジーループ: ロックを取得しようとするスレッド（またはプロセス）が、ロックが利用可能になるまで繰り返しチェックし続けるという方式

スピンロック: 特定のリソースへの同時アクセスを防ぐためのロックの一つ。ビジーループを実行し、利用可能になった瞬間にロックを取得する

### 14.7.1 sync.Mutex/sync.RWMutex

実行パス: 「メモリを読み込んで書き換える」コード？？

クリティカルセクション: 同時に実行されると問題が起きる実行コードの行

チャネルでもmutexでもロック問題は解決できる
mutexを使ってロジックが複雑になった場合はチャネルを使ってシンプルに書けないか検討する
https://github.com/golang/go/wiki/MutexOrChannel

- チャネルが有用な用途
   - データの所有権を渡す場合、作業を並列化して分散する場合、非 同期で結果を受け取る場合
- Mutex が有用な用途
   - キャッシュ、状態管理

### 14.7.2 sync.WaitGroup

- 多数の goroutine で実行しているジョブの終了待ちに使う
- ジョブ数が大量にあったり、可変個だったりする場合、チャネルよりもsync.WaitGroupのほうがよい
...
- リソースの準備ができしだい、そのリソースを待っている goroutine に通知をする (Signal() メソッド)
   - チャネルで代替可能
- 先に終わらせなければいけないタスクがあり、それが完了したら待っているすべての goroutine に通知する(Broadcast() メソッド)
   - 主な用途
   - チャネルの場合一度きりの通知にしか使えない
   - sync.Condは何度でも、goroutineの数に関わらず同じように使える

### 14.7.5 sync.Map

- 通常のmap
   - アトミックでないため、大量のgoroutineからアクセスする場合、mapの外側でロックをすることにより操作するgoroutineを1個に限定する必要
- sync.Map
   - ロックを内包し、複数のgoroutineからアクセスされても壊れないことを保証

Update的なfunctionはない
https://github.com/golang/go/issues/20884

## 14.8 sync/atomicパッケージ

- 不可分操作
   - アトミック操作
   - 情報工学においていくつかの操作を組み合わせたもので、システムの他の部分から見てそれらがひとつの操作に見えるもの
   - 次の2つの条件を満たす必要がある
      - 1. 全操作が完了するまで、他のプロセスはその途中の状態を観測できない。
      - 2. 一部操作が失敗したら組合せ全体が失敗し、システムの状態は不可分操作を行う前の状態に戻る。

### 14.8.1 メモリマップド I/O と sync/atomic

- アウト・オブ・オーダー
   - 実行ファイルに書かれた命令どおりに実行するのではなく、メモリアクセスのオーバーヘッドをうまく隠すために命令の実行順序を実行時に柔軟に入 れ替える
